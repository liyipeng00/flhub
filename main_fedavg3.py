r"""Federated Averaging (FedAvg), Scheme I in "On the Convergence of FedAvg on Non-iid Data".

Sampling and Averaging Scheme:
\bar{x} = \sum_{s\in S} 1/S x_s,
where `S` clients is selected with replacement with probablitity p=[p_1, p_2,..., p_M] per round and `p_s=n_s/n` is the weight of client `s`. 
...
# nohup python main_fedavg1.py -m mlp -d mnist -s 1 -R 100 -K 10 -M 500 -P 10 --partition exdir --alpha 2 10 --optim sgd --lr 0.05 --lr-decay 0.9 --momentum 0 --batch-size 20 --seed 1234 --log Print &

class FedServer1(FedServer):
    def __init__(self):
        pass

    def compute_prob(self, sizes, totalsize):
        prob = [sizes[i]/totalsize for i in range(len(sizes))]
        #prob[-1] = 1-sum(prob[:-1])
        return prob

    def select_clients_with_prob(self, num_clients, num_clients_per_round, prob):
        ...
        index = np.random.choice(num_clients, num_clients_per_round, replace=True, p=prob)
        sorted_index = sorted(index.tolist())
        selected_clients = []
        repeated_times = []
        for i in sorted_index:
            if i not in selected_clients:
                ...
                repeated_times[-1] += 1
        return selected_clients, repeated_times

    def aggregate(self, local_params, repeated_times):
        '''https://github.com/lx10077/fedavgpy/blob/master/src/trainers/fedavg4.py'''
        with torch.no_grad():
            param_avg = torch.zeros_like(local_params[0][0])
            assert len(local_params) == len(repeated_times) and self.num_clients_per_round == sum(repeated_times)
            for i, (param, _ ) in enumerate(local_params):
                param_avg.add_(param * repeated_times[i])
            return param_avg.div_(sum(repeated_times))
...
def customize_record_name(args):
    '''FedAvg1_M10_P10_K2_R4_mlp_mnist_exdir2,10.0_sgd0.001,1.0,0.0,0.0001_b20_seed1234_clip0.csv'''
    record_name = f'FedAvg1_M{args.M}_P{args.P}_K{args.K}_R{args.R}_{args.m}_{args.d}_{args.partition}{args.alpha[0]},{args.alpha[1]}'\
...
def main():
    ...
    prob = server.compute_prob(sizes=[client.train_feddataset.get_datasetsize(i) for i in range(args.M)], totalsize=client.train_feddataset.totalsize)
    add_log('select with probability: {}'.format(prob), flag=args.log)

    start_time = time.time()
    for round in range(args.R):
        selected_clients, repeated_times = server.select_clients_with_prob(args.M, args.P, prob=prob)
        add_log('selected clients: {}, repeated_times: {}'.format(selected_clients, repeated_times), flag=args.log)
        local_params = []
        ...
        param_avg = server.aggregate(local_params, repeated_times)
